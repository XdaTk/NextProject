...
@Component({
  ...
  template: `
    ...
    >
      <div
        *ngIf="status"
        class="card-status bg-{{ status }}"
        [class.card-status-left]="statusLeft"
      ></div>
      <div class="card-header">
        <h3 class="card-title">
          <i *ngIf="icon" class="{{ icon }}"></i> {{ header }}
        </h3>
        <div class="card-options">
            ...
            <div class="input-group">
              ...
              <span class="input-group-btn ml-2">
                <button
                  ...
                  type="submit"
                  (click)="
                    action.emit({ type: 'SEARCH', payload: search.value })
                  "
                >
          ...
          <ng-container *ngIf="!buttons">
            ...
            >
              <i
                class="fe"
                [class.fe-chevron-down]="collapsed"
                [class.fe-chevron-up]="!collapsed"
              ></i>
            </a>
            <a
          ...
          <ng-container *ngIf="buttons">
            <ng-container *ngFor="let button of buttons">
              ...
              <label class="custom-switch m-0" *ngIf="button.type === 'switch'">
                <input
                  type="checkbox"
                  [value]="button.checked"
                  class="custom-switch-input"
                  (change)="
                    action.emit({ type: 'SWITCH', payload: button.checked })
                  "
